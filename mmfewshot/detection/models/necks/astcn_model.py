...
class MultiHeadAttion(nn.Module):
    def __init__(self, num_heads=4, C_in=2, windows_size=7 * 48, dropout_rate=0.2, causality=True,
        ...
        # 处理外部数据
        self.ext_seq = nn.Sequential(nn.Conv3d(28, 128, 1, bias=False),  # 28
                                     nn.BatchNorm3d(128),
    ...
    def forward(self, multiVarInputs):
        ...
        D = inputs.shape[2]
        self.L = D
        # 降维 内部16*16*16 -> 16*1*1，外部28 —> 16
        inputs_tran = self.gap(inputs)
        if self.use_ext_in_att:
            # ext = ext.reshape(ext.shape[0], ext.shape[1] * ext.shape[-1], ext.shape[2], 1, 1)
            ext = self.ext_seq(ext.float())

            inputs_tran = torch.cat([inputs_tran, ext], dim=1)  # 32

        inputs_tran = self.merge_channel(inputs_tran)  # 64
...
class TemporalBlock(nn.Module):
    ...
    def forward(self, inputs):
        ...
        if not self.use_ext_in_att:
            ...
            a = self.relu3(out + res[:, :, downsample_shape_D, :, :])
            b = torch.cat(torch.chunk(ext[:, :, -(len(downsample_shape_D) * 2):, :, :], 2, 2), -1)
            return a, b, att_values

'''
bottlenect_channels=[8, 8, 16, 16, 32, 32, 64, 64],
num_channels=[16, 16, 32, 32, 64, 64, 128, 128],
kernel_size=[3, 3, 3],
dropout=0.0, windows_size=7 * 48, use_ext_in_att=True):
'''
...
class TemporalConvNet(nn.Module):
    def __init__(self, C_i, num_channels, bottlenect_channels, windows_size, kernel_size, dropout=0.2,
        ...
        self.num_levels = len(num_channels)  # 网络层数
        self.skip_channel = num_channels[-1]  # 输出层数
        ...
        for i in range(self.num_levels):
            ...
            # print('-----------------------------------')
            # print(in_channels, out_channels, bottlenect_channels[i], dilation_size, padding, self.block_input_len[i])
...
if __name__ == '__main__':
    ...
    windows_size = 7 * 48
    batch_size = 2
    train_set = STData(dataset_type="test", windows_size=windows_size, dir_path='./processed/TaxiBJ/')
    train_loader = data.DataLoader(dataset=train_set, batch_size=batch_size, shuffle=False)
    itertrain = iter(train_loader)

    test_data, ext_data, test_y = next(itertrain)
    # ext_data = 28 , 0-6 day; 7-8 holiday; 9 - 27 WindSpeed,WeatherTemperature
